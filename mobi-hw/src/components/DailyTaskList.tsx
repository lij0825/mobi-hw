...
import { calculateDailyResetTime } from "@/lib/utils";
import { DAILY_CATEGORIES, DEFAULT_DAILY_CATEGORY } from "@/lib/constants";
...
const DailyTaskList = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(DEFAULT_DAILY_CATEGORY);

  // 카테고리 목록
  const availableCategories = useMemo(() => {
    const categories = new Set<string>();
    dailyTaskItems.forEach((task) => {
      if (task.category) categories.add(task.category);
    });
    return [...DAILY_CATEGORIES, ...Array.from(categories)].filter(
  ...
  useEffect(() => {
    ...
    const updateTimeUntilReset = () => {
      const { timeString } = calculateDailyResetTime();
  ...
  const handleAddTask = (name: string, category: string) => {
    addDailyTask(name, category);
    setIsAdding(false);
  };

  // 작업 수정 핸들러
  const handleEditTask = (id: string, name: string) => {
    editDailyTask(id, name);
    setEditingTask(null);
  };
  ...
  const startAdding = () => {
    setEditingTask(null);
    setSelectedCategory(DEFAULT_DAILY_CATEGORY);
    setIsAdding(true);
  };
